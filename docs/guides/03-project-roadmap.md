...
| 用意済み | これから案件ごとに作るもの |
|---|---|
| 段階 1〜3 の最小例：要件・機能・クラス図・テスト仕様 | 実際の利用者の要件、必要な画面/API/DB/非機能の設計 |
| 段階 4 の一部：domain・application・メモリ adapter・7 テスト | API 契約、実 DB、HTTP、認証認可、画面 |
| 図・文書・型・依存の自動検査 | 段階 5〜6 の実環境での検証・運用 |
