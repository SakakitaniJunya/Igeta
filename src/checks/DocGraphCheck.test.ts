...
const doc = (
  ...
  `arc42: ${arc42}`, 'status: active', 'owners: [eng]', `depends_on: [${dependsOn.join(', ')}]`, 'relates_to: []', '---',
  '', `# ${title}`, '',
];
...
function makeRoot(): string {
  ...
  writeDoc(root, 'design/basic/function-list.md', doc('function-list', 'design', 'function-list', 1, '機能一覧', ['requirements']));
  return root;
}
...
describe('DocGraphCheck の本文リンク検査', () => {
  ...
  it('doc を直接持たず子ディレクトリだけのディレクトリにも索引 README を作る', async () => {
    writeDoc(root, 'design/detail/domain/overview.md', doc('domain-overview', 'design', 'domain-overview', 5, 'ドメイン概要', ['function-list']));
    await converge(root);
  ...
  it('ディレクトリ索引は <context>.md を <context>-<aspect>.md より先に並べる', async () => {
    writeDoc(root, 'design/detail/domain/catalog.md', doc('domain-catalog', 'design', 'domain-model', 5, 'catalog', ['function-list']));
    writeDoc(root, 'design/detail/domain/catalog-pricing.md', doc('domain-catalog-pricing', 'design', 'domain-model', 5, 'catalog / 料金', ['function-list']));
    await converge(root);
  ...
  it('ディレクトリ索引に各文書の TL;DR を説明として出す (強調・リンク・コードは剥がす)', async () => {
    writeDoc(root, 'design/detail/domain/catalog.md', [
      ...doc('domain-catalog', 'design', 'domain-model', 5, 'catalog', ['function-list']),
    ...
    assert.match(detail, /^- \[domain\/\]\(domain\/README\.md\) — \*\*domain — 索引\*\* — ドメインクラス図。1 コンテキスト 1 ファイル。$/m);
  });
...
describe('DocGraphCheck の階層索引と配置検査', () => {
  let root: string;
  const converge = async (r: string): Promise<void> => {
    assert.equal((await run(r, 'write')).status, 0);
    assert.equal((await run(r, 'write')).status, 0);
  };
  const business = (id: string, title: string, extra: readonly string[] = []): string[] => [
    '---', `id: ${id}`, `title: ${title}`, 'type: business', 'status: active', 'owners: [eng]', ...extra, '---',
    '', `# ${title}`, '',
  ];
  ...
  it('ディレクトリ索引は depends_on の木で出す (子は親の下に字下げ、ディレクトリ外の上流は ← 上流)', async () => {
    ...
    writeDoc(root, 'business/finance/tax.md', business('fin-tax', '税務手順', ['depends_on: [fin-plan]']));
    writeDoc(root, 'business/finance/old-plan.md', business('fin-old-plan', '旧 収支計画', ['status: superseded', 'superseded_by: fin-plan']));
    writeDoc(root, 'business/finance/grants/sogyo.md', business('fin-grant-sogyo', '創業助成金', ['depends_on: [fin-plan]']));
    await converge(root);
    const check = await run(root, 'check');
    assert.equal(check.status, 0, check.detail);
    ...
    const indent = (needle: string): number => lines[at(needle)]?.match(/^ */)?.[0].length ?? -1;
    assert.equal(indent('[policy.md]'), 0, index);
    assert.equal(indent('[plan.md]'), 2, index);
    assert.equal(indent('[tax.md]'), 4, index);
    assert.equal(indent('[old-plan.md]'), 4, index); // superseded_by で新の下にぶら下がる
    assert.ok(at('[policy.md]') < at('[plan.md]') && at('[plan.md]') < at('[tax.md]'), index);
    assert.match(index, /\[policy\.md\].*← 上流: \[requirements\]\(\.\.\/\.\.\/product\/requirements\.md\)/);
    assert.match(index, /\[old-plan\.md\].*_\(superseded\)_/);
    assert.match(index, /^- \[grants\/\]\(grants\/README\.md\)/m);
    assert.doesNotMatch(index, /\| ファイル \| タイトル \|/);
  });

  it('負例: 上流も下流も無い文書は --check で落ちる (一覧に足すだけでは登録にならない)', async () => {
    writeDoc(root, 'business/finance/lonely.md', business('fin-lonely', '孤立文書'));
    const written = await run(root, 'write');
    assert.equal(written.status, 0, written.detail); // --write は書いたうえで warning
    assert.ok(written.warnings.some((w) => w.includes('fin-lonely') && w.includes('階層に無い')), written.detail);
    await converge(root);
    const result = await run(root, 'check');
    assert.equal(result.status, 1, result.detail);
    const hit = result.violations.find((v) => v.message.includes('fin-lonely'));
    assert.ok(hit, result.detail);
    assert.match(hit.message, /^\[graph\] fin-lonely \(docs\/business\/finance\/lonely\.md\) が階層に無い/);
  });

  it('根になれる種別 (requirements / adr / runbook / guide など) は上流が無くても通る', async () => {
    writeDoc(root, 'runbooks/deploy.md', [
      '---', 'id: deploy-runbook', 'title: デプロイ手順', 'type: runbook', 'kind: runbook',
      'status: active', 'owners: [eng]', 'depends_on: []', 'relates_to: []', '---', '', '# デプロイ手順', '',
    ]);
    await converge(root);
    const result = await run(root, 'check');
    assert.equal(result.status, 0, result.detail);
  ...
  it('負例: depends_on の循環は落ちる (木にならない)。索引からは消さない', async () => {
    writeDoc(root, 'business/finance/a.md', business('fin-a', 'A', ['depends_on: [fin-b]']));
